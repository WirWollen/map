...
public interface WorkloadRepo extends CrudRepository<WorkloadEntity, Long> {
    ...
    @Query(value = "SELECT COUNT(*) FROM workload WHERE time > ?1 AND time < ?2 AND active = true", nativeQuery = true)
    Integer calculateClients(LocalDateTime startTime, LocalDateTime endTime);
}
